...
class AvroKafkaSerdeSpec extends WordSpec with Matchers with MockFactory with DefaultJsonProtocol {
  ...
  val schemaV2: Schema = new Parser().parse(
    """
      |{
      |     "type": "record",
      |     "namespace": "com.example",
  ...
  "kafka avro serializer" when {
    ...
    "deserializing" should {
      ...
      "successfully process incoming bytes and produce a valid payload" in {
        schemaRegistryServiceStub.schema _ when("testStrainA", Version("0.1.0")) returns Future.successful(schemaV1)
        ...
        sut.deserializer().deserialize("", headers, bytes) shouldEqual Payload("testStrainA", Version("0.1.0"),
          """
            | {
            |   "first": "valA",
            |   "last" : 4
      ...
      "successfully process incoming bytes serialized using a wider schema and produce a valid payload" in {
        schemaRegistryServiceStub.schema _ when("testStrainA", Version("0.1.0")) returns Future.successful(schemaV2)
    ...
    "serializing" should {
      ...
      "successfully process payload with applicable schema" in {
        ...
        val payload = Payload("testStrainA", Version("0.1.0"),
          """
            | {
            |   "first": "valA",
            |   "last" : 4
      ...
      "successfully process with wider schema " in {
        ...
        sut.serializer().serialize("", new RecordHeaders(), payload) shouldEqual Array[Byte](8, 118, 97, 108, 65, 8, 118, 97, 108, 66, 8)
      }
